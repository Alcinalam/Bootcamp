...
public class Order {
  ...
  public static void main(String[] args) {
    // int orderId, int customerId, double amount,LocalDate orderDate
    ...
    // 1. filter orders of that customer with total amount of all orders >1100
    // approach: groupby sum -> Map -> stream -> entry -> flatmap -> orders(mapping)
    List<Order> result2 = orders.stream() //
        .collect(Collectors.groupingBy(Order::getCustomerId,
            Collectors.summingDouble(Order::getAmount))) // Map
        .entrySet().stream() // stream the whole Map
